...
knitr::opts_chunk$set(echo = TRUE, dev = c("pdf"), fig.path = "Rauch2019_to_heatmap/", fig.dim = c(4, 8))
```

## Load count data from GEO and convert gene ids

```{r}
read.geo <- function(file_url) {
  con <- gzcon(url(file_url))
  txt <- readLines(con)
  return(read.delim(textConnection(txt), header=T))
}
```


```{r}
rauch_file = "ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE113nnn/GSE113253/suppl/GSE113253%5FGeneExpr%5FAT%2Etxt%2Egz"

expr = read.geo(rauch_file)
#head(expr) 
dim(expr)
```
...
mart <- biomaRt::useMart(biomart = "ensembl",
  ...
  host = "ensembl.org")
#look for GO filter
head(searchFilters(mart = mart, pattern = ".ef.eq"), n=20)
head(searchAttributes(mart = mart, pattern = ".ef.eq"), n=20)

ens = getBM(c("external_gene_name", "ensembl_gene_id","refseq_mrna"), 
              filters = "refseq_mrna",
              values = expr$RefSeqID,
              mart = mart)

nrow(ens) #15644 id found
length(unique(ens$ensembl_gene_id)) #15619 unique ensembl ids
length(unique(ens$refseq_mrna)) #14589 unique refseqIDs

nc_ens = getBM(c("external_gene_name", "ensembl_gene_id", "refseq_ncrna"), 
              filters = "refseq_ncrna",
              values = expr$RefSeqID,
              mart = mart)

nrow(nc_ens) #2487 ncrna
length(unique(nc_ens$ensembl_gene_id)) #2478 unique ensembl ids
length(unique(nc_ens$refseq_ncrna)) #2300 unique refseqIDs
```

```{r}
#First combine mRNA and ncRNA
colnames(ens) = c("gene_name","gene","RefSeqID")
colnames(nc_ens) = c("gene_name","gene","RefSeqID")
ids = rbind(ens, nc_ens)
dim(ids)

#Add ensembl Id column
nexpr = merge(expr,ids, by="RefSeqID",all=F)
dim(nexpr) #18132
head(nexpr)
```
...
```{r save}
write.table(nexpr, "../03limma/rauch2019_gene_list.tab", sep="\t",row.names=FALSE)
```


```{r load_saved_file}
nexpr = read.delim("../03limma/rauch2019_gene_list.tab", header = T)
...
length(order)
```
## Supp Figure 6c - Ribosomal Proteins
