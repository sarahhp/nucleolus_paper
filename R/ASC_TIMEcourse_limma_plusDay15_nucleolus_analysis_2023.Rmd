...
intermediate files (merge of several experiments): 
...
d2_info$Tube.Label = NULL
head(d2_info)

#Checked that FC table and d2_info are in the same order
sum(colnames(d2_tab)[7:ncol(d2_tab)] == d2_info$sample_id) == nrow(d2_info)
```
...
d1_info = read.delim("../sample_info/donor1_info.csv", header=TRUE, stringsAsFactors = FALSE, sep = ",")
d1_info = d1_info[order(d1_info$sample_id),]
head(d1_info)

#Checked that FC table and d1_info are in the same order
sum(colnames(d1_tab)[7:ncol(d1_tab)] == d1_info$sample_id) == nrow(d1_info)
```
...
#Checke that FC table and d1_info are in the same order
sum(colnames(day15_tab)[7:ncol(day15_tab)] == day15_info$sample_id) == nrow(day15_info)
```
Edits to day15 file to fit: 
- remove D from time; add dash to donor
- change column names: cell_type to exp
- remove bio.condition
...
colnames(rpkm) = paste("D", 
                       gsub("D(1|2)","",filt_series$samples$group),
                       "_rep",filt_series$samples$rep, sep="")
custom_order = gsub("[[:alpha:]]*","", gsub("\\.(A|G).*","",colnames(rpkm)))
rpkm = rpkm[order(as.integer(custom_order), colnames(rpkm))]



colnames(rpkm) = gsub("D-2", "PROLIF",colnames(rpkm))
head(rpkm)
